...
def get_table_html(title, data, sortable_columns=None):
    ...
    logger.debug(f"get_table_html: title={title}, data_length={len(data) if data else 0}")
    ...
    for row_data in data:
        ...
        logger.debug(f"get_table_html: row_data={row_data}, tds={tds}")
    ...
    logger.debug(f"get_table_html: tbody_rows length={len(tbody_rows)}")
